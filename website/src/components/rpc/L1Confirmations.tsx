...
export const EthCallFixed = () => {
  ...
  const handleEthCall = async () => {
    ...
    const abi = ["function getL1Confirmations(bytes32 blockHash) external view returns (uint64 confirmations)"];
    const iface = new ethers.utils.Interface(abi);
    const data = iface.encodeFunctionData("getL1Confirmations", [blockHash]);
    ...
    try {
      ...
      if (json.error) {
        ...
        setResult([`Confirmations: ${formattedResult}`]);
      }
    } catch (error) {
  ...
  return (
    <div style={styles.container}>
      ...
      <input id="blockhash-input" style={styles.input} type="text" value={blockHash} onChange={e => setBlockHash(e.target.value)} />
